from random import randint
import turtle

class Point:
    
    def __init__(self, x, y):
        self.x = x
        self.y = y
    
    def falls_in_rectangle(self, rectangle):
        if rectangle.point1.x < self.x < rectangle.point2.x and rectangle.point1.y < self.y < rectangle.point2.y:
            return True
        else:
            return False

class GuiPoint(Point):

    def draw(self, canvas, size=5, color='red'):
        canvas.penup()
        canvas.goto(self.x, self.y)
        canvas.pendown()
        canvas.dot(size, color)

class Rectangle:
    
    def __init__(self, point1, point2):
        self.point1 = point1
        self.point2 = point2

    def area(self):
        return (self.point1.x - self.point2.x) * (self.point1.y - self.point2.y)

class GuiRectangle(Rectangle):

    def draw(self, canvas):

        # Move turtle (drawing)
        canvas.penup() # Lift pen
        canvas.goto(self.point1.x, self.point1.y)

        # Draw rectangle
        canvas.pendown() # Put down pen
        canvas.forward(self.point2.x - self.point1.x)
        canvas.left(90)
        canvas.forward(self.point2.y - self.point1.y)
        canvas.left(90)
        canvas.forward(self.point2.x - self.point1.x)
        canvas.left(90)
        myturtle.forward(self.point2.y - self.point1.y)

# Main flow

# Creates the random rectangle
rectangle = GuiRectangle(Point(randint(0,400), randint(0,400)), Point(randint(10,400), randint(10,400)))

# Show rectangle coordinates
print("Rectangle coordinates: ", rectangle.point1.x, ",", rectangle.point1.y, "and", rectangle.point2.x, ",", rectangle.point2.y)

# Ask user for coordinates and area guessed
user_point = GuiPoint(float(input("Guess X: ")), float(input("Guess Y: ")))
user_area = int(input ("Can you guess the area of the rectangle?: "))

# Game results
print ("Your point was inside the rectangle: ", user_point.falls_in_rectangle(rectangle))
if user_area == rectangle.area:
    print ("You guessed!!!")
else:
    print ("Not quite, the area of the rectangle is: ", rectangle.area(), " you were off by: ", rectangle.area() - user_area)

# Show rectangle
myturtle = turtle.Turtle()
rectangle.draw(canvas = myturtle)
user_point.draw(canvas = myturtle)
turtle.done()